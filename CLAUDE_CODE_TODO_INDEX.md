...
| `DEVPLAN.md:47` | 盘点 OPi5 与 PC 上已有 RKNN 仓库 | Claude Code 在 PC/OPi5 上列出可运行 demo、模型文件、依赖版本；选择第一个可接入的 demo。 | 形成 `edge/opi5-ai/models/README.md` 和 `tests/opi5/YYYY-MM-DD_rknn_inventory.md`。 |
| `docs/09_OrangePi5_RKNN本地AI推理与解释方案.md:21` | 盘点 PC 和 OPi5 上已有 RKNN 仓库与可运行 demo | Claude Code 在本机与 OPi5 上列目录、运行已有 demo，选择最小可接入模型。 | 产出 RKNN 资产清单：模型名、输入尺寸、类别、运行命令、是否通过。 |
...
| `docs/11_测试计划与风险矩阵_端边协同版.md:25` | 补齐 i.MX6ULL 单模块测试结果 | 按 Task02–04 逐项执行并填写结果。 | 每项至少有一条测试记录，失败也记录原因。 |

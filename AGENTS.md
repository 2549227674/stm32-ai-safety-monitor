...
| 总体阶段 | Task02 进行中：WSL SDK 与 hello 交叉编译已通过，i.MX6ULL SSH/部署本轮复测失败 |
| 新主线 | i.MX6ULL Pro + Orange Pi 5 + Flask |
| 旧主线 | STM32 + ESP32-CAM 归档为 legacy |
| 当前执行器 | 本地 Codex |
| 首要目标 | 恢复 WSL/PC→i.MX6ULL SSH 后重跑 `hello_imx6ull` 部署与板端运行 |
...
|---|---|---|---|---|
| i.MX6ULL SSH | PC/WSL 可登录 | 当前复测失败（用户曾人工验证 B 方案可登录） | `tests/imx6ull/2026-06-06_toolchain_ssh.md` | Task02 |

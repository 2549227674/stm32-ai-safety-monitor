...
| MG90S 云台 | pan/tilt 可控 | 已验证（分时模式 + Task08-A 三点巡检扫描通过：pan 60/90/120°, tilt 90°, 无抖动/卡死/压降） | `tests/imx6ull/2026-06-07_pan_tilt_demo.md` | Task08-A |
...
| OPi5 SSH / AI mock 服务 | 登录 + `/health`、`/api/infer/vision` mock 可用 | Task05-A 已验证（Python 3.10.12、Flask 2.2.5、mock HTTP 200、`control_allowed=false`） | `tests/opi5/2026-06-07_ai_mock_service.md` | Task05-A |
| OPi5 Qwen3-VL 2B 真实 AI | `/health` + `/api/infer/vision` 真实 VLM 推理 | Task05-B 已通过（Qwen3-VL 2B rknn+rkllm、中文场景描述、risk_mapping、mock 回归、`control_allowed=false`） | `tests/opi5/2026-06-07_qwen3vl_real_ai.md` | Task05-B |
...
| OLED SSD1306 | /dev/i2c-0 addr 0x3C 状态屏 | 已验证（i2cdetect 0x3C，oled_test 刷屏，imx_safetyd OLED_ENABLE=1 loop 刷新） | `tests/imx6ull/2026-06-07_p1_oled_status_screen.md` | Task11-A |
| relay 继电器 | PCA9685 CH5 KY-019 | 已验证（active high，默认 OFF，ALARM 动作，RELAY_ENABLE=0|1） | `tests/imx6ull/2026-06-07_p1_relay_ch5.md` | Task11-B |
| SoC 温度 | thermal_zone0 芯片温度 | 已验证（46.6°C，SOC_TEMP_WARN_C 可配置，device_health WARN/NORMAL，不触发 ALARM） | `tests/imx6ull/2026-06-07_p1_soc_temp_health.md` | Task11-C |
| pump 双负载 | PCA9685 CH6 → MOS → 水泵+水枪 | 已验证（active high，default OFF，ALARM 动作，双负载并联短时演示通过） | `tests/imx6ull/2026-06-07_p1_pump_water_tank.md` | Task11-D |
...
| Task07-C C 版 imx_safetyd | 已通过 | `tests/integration/2026-06-07_imx_safetyd_c.md`、`edge/imx6ull-controller/src/imx_safetyd.c`、`edge/imx6ull-controller/systemd/imx-safetyd.service` | C 版主控程序已交叉编译并部署到 i.MX6ULL，支持 `once/loop/flush`、读取真实 PIR gpio117、NORMAL/VERIFY/FAULT 最小状态机、VERIFY 抓拍、OPi5 mock AI、干净 fallback AI、Flask 上报、pending spool、flush 到 sent、运行日志和 status JSON；`control_allowed=false`；当前板端无 `systemctl`，systemd 模板仅入库备用；本轮 PIR 仅测到空闲 raw=0，VERIFY 使用 `FORCE_VERIFY=1`。 |
| Task07-C2 init.d 启动管理 | 已通过 | `tests/integration/2026-06-07_imx_safetyd_initd.md`、`edge/imx6ull-controller/init.d/S99imx-safetyd`、`scripts/install_imx_safetyd_initd.sh` | BusyBox/SysV `init.d` 适配完成，支持 `start/stop/restart/status`、PID 文件、日志追加、重复启动防护；板端 `/etc/init.d/rcS` 会执行 `S??*`，具备开机自启条件；`/etc/edge-ai-safety-monitor/imx-safetyd.env` 由板端手动创建，未入库。 |
| Task05-B Qwen3-VL 2B 真实 AI | 已通过 | `tests/opi5/2026-06-07_qwen3vl_real_ai.md`、`edge/opi5-ai/service/qwen3vl_backend.py`、`edge/opi5-ai/service/risk_mapping.py`、`edge/opi5-ai/service/qwen3vl_single_shot.cpp` | Qwen3-VL 2B 在 OPi5 上通过 rknn-llm C++ demo 部署，vision encoder (RKNN) + LLM (RKLLM W8A8) 真实推理通过；`/health` 和 `/api/infer/vision` 返回真实 VLM 结果；`vlm_result/explanation` 向后兼容新增；risk_mapping 支持中文否定检测；AI_BACKEND=mock 回归通过；`control_allowed=false`；当前通过子进程调用，端到端 ~13s。 |
| Task05-C Qwen3-VL 持久化 + i.MX 回归 | 已通过 | `tests/opi5/2026-06-07_qwen3vl_persistent_regression.md`、`edge/opi5-ai/service/qwen3vl_worker.cpp`、`edge/opi5-ai/service/qwen3vl_worker_client.py` | Qwen3-VL 从 single-shot 优化为持久化 worker（stdin/stdout 协议），后续请求 ~8.4s（vs single-shot ~13s）；i.MX imx_safetyd 端边回归通过（capture→OPi5 qwen3vl→Flask 201）；risk_hint=0/vlm_result/control_allowed=false 正确；AI_BACKEND=mock 回归通过。 |
...
| Task11-A OLED 状态屏 | 已通过 | `tests/imx6ull/2026-06-07_p1_oled_status_screen.md` | i2cdetect 0x3C 可见；oled_test 刷屏通过；imx_safetyd OLED_ENABLE=1 loop 刷新 state/risk_score/sensors/actuators；OLED 失败不影响主控；PCA9685 RGB 不受影响。 |
| Task11-B relay CH5 | 已通过 | `tests/imx6ull/2026-06-07_p1_relay_ch5.md` | KY-019 5V 模块，默认 OFF；PCA9685 CH5 active high（duty=4095 吸合）；ALARM 时 relay=1，NORMAL/VERIFY/FAULT 时 relay=0；all_off 确保退出释放；RELAY_ENABLE=0|1 可选。 |
| Task11-C SoC 温度热健康 | 已通过 | `tests/imx6ull/2026-06-07_p1_soc_temp_health.md` | thermal_zone0 可读 46.6°C；device_health WARN/NORMAL 可配置阈值；soc_temp 不触发 ALARM、不改 risk_score、不驱动 relay；OLED 显示温度和健康状态；事件 JSON 含 soc_temp 和 device_health。 |
| Task11-D Pump 双负载水箱 | 已通过 | `tests/imx6ull/2026-06-07_p1_pump_water_tank.md` | PCA9685 CH6 双负载（水泵+水枪发射器并联），active high，默认 OFF；ALARM 时 pump=1 双负载动作；NORMAL/VERIFY/FAULT 时 pump=0；all_off 确保退出关闭；PUMP_ENABLE=0|1 可选。 |
| Task11 P1 扩展 | 主体完成 | `CLAUDE_CODE_TASK_11_imx6ull_p1_oled_relay_soctemp_pump.md`、CANONICAL 0.7 | 11-A/11-B/11-C/11-D 全部通过；P1 扩展主体完成 |
...
| Task12-I OPi5 Qwen3-VL 真实 AI 自启动 | 已通过 | `tests/opi5/2026-06-08_qwen3vl_autostart_real_ai.md` | systemd 自启动，默认 Qwen3-VL 真实 AI（非 mock），API 内嵌 worker 管理，重启后自动恢复，i.MX→OPi5 infer ~5s；mock 保留为手动回退 |

...
void setup()
{
    ...
    LOG_PRINTLN();
    LOG_PRINTLN("=== ESP32-CAM Safety Monitor UART1 Bridge ===");
    LOG_PRINTLN("[mode] stage 5: STM32 UART1 JSON -> HTTP event");
    ...
    LOG_PRINTF("[uart] uart=1 rx_pin=GPIO%d tx_pin=GPIO%d baud=%lu\n",
               STM32_UART_RX_PIN,
               STM32_UART_TX_PIN,

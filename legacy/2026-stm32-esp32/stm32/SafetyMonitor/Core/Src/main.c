...
#define APP_COMM_TX_PERIOD_MS 1000U
#define DIAG_PA2_GPIO_TOGGLE  0U
/* USER CODE END PD */
...
static uint32_t s_comm_tx_count = 0U;
static HAL_StatusTypeDef s_comm_last_status = HAL_OK;
/* USER CODE END PV */
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if ((HAL_GetTick() - s_comm_last_tx_ms) >= APP_COMM_TX_PERIOD_MS)
    {
        s_comm_last_tx_ms = HAL_GetTick();
        s_comm_last_status = AppComm_SendStm32Test();
        if (s_comm_last_status == HAL_OK)
        {
            ++s_comm_tx_count;

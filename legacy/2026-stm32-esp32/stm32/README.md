...
| Stage 5 / Task 04 | USART1 → ESP32-CAM UART 桥接 | 代码已准备，待联调 | PA9=USART1_TX、PA10=USART1_RX，115200 8N1；当前只发送固定 `STM32_TEST` JSON；PA2/USART2 因逻辑分析仪未得到稳定 UART 波形，阶段 5 暂时放弃 |
...
`app_comm` 只负责组装固定 `STM32_TEST` 事件 JSON，并通过 `USART1` 发送给 ESP32-CAM。每帧以 `\n` 结尾，第一版不读取 GPIO、不控制执行器、不实现 `risk_score` 或 `safety_fsm`，也不启用 DHT11。

USART1 配置由用户通过 CubeMX 生成：
...
PA2/USART2 在逻辑分析仪上未得到稳定 115200 UART 波形，阶段 5 暂时放弃。`main.c` 当前每 1 秒调用一次 `AppComm_SendStm32Test()`。Keil 编译前需要确认工程中已加入 `Core/Src/app_comm.c`。

最小接线：

```text
STM32 PA9 / USART1_TX -> ESP32-CAM U0R / RX / GPIO3

...
| Stage 5 / Task 04 | STM32 → ESP32-CAM UART 桥接 | 代码已准备，待联调 | USART1：PA9 TX、PA10 RX，115200 8N1 | 待 Keil 编译、烧录、接线和 Dashboard 验收 | 尚未实测通过 | 固定发送 `STM32_TEST` JSON；PA2/USART2 因逻辑分析仪未得到稳定 UART 波形，阶段 5 暂时放弃 |
...
- CubeMX 已由用户启用 USART1：PA9=TX、PA10=RX、115200 8N1。
- PA2/USART2 逻辑分析仪未得到稳定 115200 UART 波形，阶段 5 暂时放弃。
- STM32 新增 `app_comm.h/c`，通过 `HAL_UART_Transmit()` 发送固定 `STM32_TEST` JSON，帧尾为 `\n`。
- `main.c` 每 1 秒发送一次测试事件。

...
| 引脚 | 计划功能 | 状态 | 说明 |
|---|---|---|---|
| PA9 | USART1_TX | 已配置 / 待联调 | Stage 5 / Task 04：STM32 发送 `STM32_TEST` JSON 到 ESP32-CAM U0R/RX/GPIO3；尚未完成硬件联调验收 |
| PA10 | USART1_RX | 已配置 / 待联调 | Stage 5 / Task 04：USART1 RX 预留，当前主要验证 STM32 PA9 TX → ESP32-CAM RX |
| PA2 | USART2_TX | 暂时放弃 | Task 04 曾用于 STM32 → ESP32-CAM UART；逻辑分析仪未得到稳定 115200 UART 波形，阶段 5 暂不使用 |
| PA3 | USART2_RX | 暂时放弃 | Task 04 USART2 RX 预留同步放弃，阶段 5 改用 USART1 PA9/PA10 |
...
| USART1 桥接 | Task 04 改用 PA9/PA10 为 3.3V TTL UART；PA9 接 ESP32-CAM U0R/RX/GPIO3 时必须共地，ESP32-CAM 继续由 USB 烧录底座供电，不得由 STM32/ST-Link 3.3V 供电 |
| USART2 桥接 | PA2/PA3 因逻辑分析仪未得到稳定 UART 波形，阶段 5 暂时放弃 |

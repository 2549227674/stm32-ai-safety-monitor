...
| PIR | PB15 | 已完成 |
| MQ-2 DO 软件链路 | PB14 | 软件模拟通过；PB14 为 FT 数字输入，实物 DO 接入前需确认 DO 高电平不超过 5V 且 GPIO 输入禁用内部上下拉 |
...
- [x] ESP32-CAM 可烧录。
- [ ] Serial Monitor 115200 可看到 WiFi 连接状态。（串口 Monitor 输出异常，不作为阻塞项）
- [x] ESP32-CAM 与服务器在同一局域网。（使用手机热点，非校园网）
- [x] Flask 服务器防火墙允许 5000 端口。
- [x] 每 5 秒上报一条 `state=TEST` 事件。
- [x] Dashboard 可看到 ESP32-CAM 测试事件。
- [ ] 服务器断开时 ESP32-CAM 不死循环卡死，能继续重试。（未单独测试，但代码有超时和重试逻辑）

### 实测记录（2026-06-04）

- **网络**：校园网有认证问题，使用手机热点完成测试。
- **设备**：ESP32 设备名 `esp32-72AA3C`，MAC `8c:94:df:72:aa:3c`。
- **接线**：仅 ESP32-CAM + USB 烧录底座，未接 STM32、UART、传感器、执行器。
- **Flask**：连续出现 `POST /api/events HTTP/1.1 201`。
- **Dashboard**：`state=TEST`、`risk_score=0`、`device_id=labbox_001`、`seq` 持续递增，sensors/actuators 全为 0。
- **串口 Monitor**：输出异常，但 HTTP 与 Dashboard 链路已通过，不作为阶段 4 阻塞项。
- **结论**：阶段 4 通过。详见 `tests/esp32cam_http_test_record.md`。
...
3. MQ-2 PB14：先继续用 PB14 → GND/3.3V 软件模拟；实物 DO 接入前确认 DO 高电平不超过 5V 且 PB14 GPIO 输入禁用内部上下拉。AO 接 ADC 前必须等分压电阻到位。
...
4. 不接 220V 强电
5. 不把 MQ-2 AO 直接接入 STM32 ADC，除非分压到位；MQ-2 DO 仅在 FT 输入、电平确认和禁用内部上下拉条件满足时接入
```

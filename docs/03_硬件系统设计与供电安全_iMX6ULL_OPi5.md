...
OPi5 可通过 USB WiFi 网卡（0bda:0179 / RTL8188ETV）连接手机热点，减少一条 OPi5 有线网线。此时 Windows 主机同时连接 i.MX 有线网络和 OPi5 WiFi 网络，通过 portproxy 转发 i.MX 的 AI 请求。

```text
i.MX6ULL (192.168.137.110) → Windows (192.168.137.1:18080) → OPi5 WiFi (10.96.98.38:8080)
```

此方案为可选优化，不是必须硬件。全有线拓扑仍保留为最终演示回退。详见 `CANONICAL_DECISIONS.md` 第 0.8 节。

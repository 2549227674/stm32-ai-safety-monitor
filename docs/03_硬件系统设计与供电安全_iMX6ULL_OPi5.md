...
| I2C 地址 | 0x3C |
| 逻辑电压 | 3.3V |
| 与 PCA9685 共总线 | 0x40 已用，0x3C 待验 |
| 状态 | 待验证 |

> **[CLAUDE_CODE_TODO | VERIFY]** I2C 0x40/0x3C 共存：OLED 接入后 i2cdetect 同时 ACK，刷屏不影响舵机/RGB
> - 期望产物/操作：`i2cdetect -y 0` 同时看到 0x40/0x3C；OLED 显示文字时舵机/RGB 不受影响。
> - 回填位置：docs/03；tests/imx6ull。
> - 验收：两个地址同时 ACK，互不干扰。

...
| `VERIFY` | 复核中 | 本地传感器触发，需要视觉确认 | 抓拍、请求本机 `opi5_ai_service`，执行器保持安全态 |
...
OPi5 Task13 起真实驱动蜂鸣器（GPIO35，active LOW）与 RGB（GPIO48/50/52，active HIGH），水枪 MOS 使用 GPIO138 active HIGH；NORMAL=绿、VERIFY=蓝、ALARM=红+蜂鸣器响，FAULT=紫/黄间歇。PCA9685 当前不可用，继电器/水泵类负载按 GPIO/MOS/PWM 直控方案处理。AI 服务/Flask 不控制执行器；只有本地安全主控进程 `opi5_safetyd` 控制执行器，`control_allowed=false`。
...
| 摄像头不可用 | 不抓拍，保留传感器事件 |
| PCA9685 不可用 | 当前已降级为 GPIO/PWM 直控，PCA9685 不作为主线依赖 |
...
## 6.11 历史 i.MX 阶段：Task07-C C 版 imx_safetyd 落地状态（已完成验证，不再部署）
...
- AI 正常时按 `risk_score = min(10, 3 + risk_hint)` 融合；AI 服务不可达时生成干净 fallback `ai_result.ok=false/mode=offline/control_allowed=false`，`risk_score=4`。
...
## 6.12 历史 i.MX 阶段：Task10 P0 传感器/执行器真实化（已完成验证）

Task10 已把 imx_safetyd 中 door/flame/mq2 的固定 0 替换为真实 GPIO 读取，并真实驱动蜂鸣器（gpio122）与 RGB（gpio121/123/124）。引脚分配以 `CANONICAL_DECISIONS.md` 第 0.6 节为唯一事实源。
...
## 6.13 历史 i.MX 阶段：Task11 P1 OLED / 继电器 / SoC 温度 / 隔离水箱（已完成验证）
...
- AI 服务/Flask 不驱动 relay/pump；`control_allowed=false`。
- PCA9685 只接受本地安全主控进程控制。
- 不接 220V；水与板卡物理隔离。

详见 `CLAUDE_CODE_TASK_11_imx6ull_p1_oled_relay_soctemp_pump.md`。

## 6.14 OPi5 Task13 当前主线状态摘要

Task13 已完成 OPi5 controller migration，GPIO/PWM/MOS 全部验证：

| 模块 | OPi5 引脚 | 逻辑 | 说明 |
|---|---|---|---|
| flame 火焰 | GPIO28 | active LOW | flame=1 → ALARM |
| MQ-2 烟雾 | GPIO49 | active LOW | mq2=1 → ALARM |
| PIR 人体红外 | GPIO139 | active HIGH | pir=1 → VERIFY |
| buzzer 蜂鸣器 | GPIO35 | active LOW | 0=响 |
| RGB-R 红灯 | GPIO48 | active HIGH | 1=亮 |
| RGB-G 绿灯 | GPIO50 | active HIGH | 1=亮 |
| RGB-B 蓝灯 | GPIO52 | active HIGH | 1=亮 |
| 水枪 MOS | GPIO138 | active HIGH | 1=开启 |
| 舵机 | pin7 / PWM15 / pwmchip4 | PWM | 80–90 度有效范围 |
| OLED | I2C5 0x3C | I2C | SSD1306 状态屏 |
| MPU6050 | I2C5 0x68 | I2C | 可选振动监测 |

- PCA9685 确认为模块问题（4 个均未 ACK），当前主线改用 GPIO/PWM 直控
- 引脚配置已更新到 `edge/opi5-controller/src/opi5_safetyd.c`
- 证据见提交 `533f226`

## 6.15 历史 i.MX 阶段：Task07-D 后续原生化增强（可选，不阻塞 MVP）

Task07-C 当前已实现 C 版 imx_safetyd，支持 once/loop/flush、真实 gpio117 空闲读取、NORMAL/VERIFY、AI 服务 offline fallback、Flask spool/flush。

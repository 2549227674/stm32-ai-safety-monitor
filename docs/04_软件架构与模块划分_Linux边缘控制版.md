...
> **[CLAUDE_CODE_TODO | FILL]** 填写 SDK 环境脚本、工具链三元组、sysroot
> - 为何 GPT 给不了：i.MX6ULL SDK 不在沙箱中。
> - 期望产物/操作：在 WSL 中定位 `environment-setup-*`，确认 `$CC`、`$CXX`、`$SDKTARGETSYSROOT`，写入本地配置。
> - 回填位置：docs/04 第 4.7；Task02
> - 验收：hello 能交叉编译并在 i.MX6ULL 运行。

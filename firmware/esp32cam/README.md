...
- 默认关闭 Serial 调试输出，避免 UART0 同时承担下载/日志和 STM32 输入时互相干扰。
- WiFi 断开后自动重连。
...
Task 04 默认使用 ESP32-CAM 板子丝印 `RX` / `U0R`，也就是 UART0 RX / GPIO3，接收 STM32 `PA2 / USART2_TX`。`STM32_UART_TX_PIN=1` 只用于标注 UART0 TX / U0T，本任务不需要 ESP32-CAM 向 STM32 发送数据。

只有在复测阶段 4 的 ESP32-CAM 单独 HTTP 测试时，才把 `ENABLE_PERIODIC_TEST_EVENT` 设为 `1`。Task 04 UART 桥接验收时保持 `0`。

如需临时观察串口日志，可把 `ESP32CAM_DEBUG_LOG` 设为 `1`，但 UART0 会同时用于日志和 STM32 输入，正式验收建议保持 `0`。
...
如果 ESP32-CAM 插在 USB 烧录底座上，`U0R/RX/GPIO3` 通常与 USB-UART 芯片共用。烧录 ESP32-CAM 时建议先断开 `STM32 PA2 -> ESP32-CAM U0R/RX` 这根线，烧录完成并复位运行后再接回。
...
不要用 STM32 3.3V 或 ST-Link 3.3V 给 ESP32-CAM 供电。STM32F103 和 ESP32-CAM UART 都是 3.3V TTL，不要在这条链路中接入 5V UART。如果 ESP32-CAM 使用外部 5V 降压板供电，不要同时接 USB 烧录底座供电，避免两路电源互相顶住。
...
- 确认烧录底座是否支持自动下载。
- 烧录前先断开 `STM32 PA2 -> ESP32-CAM U0R/RX`，避免 STM32 输出占用下载串口。
- 如果没有自动下载，先连接 `IO0` 到 `GND`，按 reset 后上传；上传完成后断开 `IO0` 与 `GND`，再 reset 运行。
- 烧录完成并复位运行后，再接回 `STM32 PA2 -> ESP32-CAM U0R/RX`。
...
[uart] rx_pin=U0R/GPIO3 tx_pin=U0T/GPIO1 baud=115200
...
6. 烧录完成后，将 STM32 `PA2 / USART2_TX` 接到 ESP32-CAM `U0R/RX/GPIO3`，并连接公共 GND。
...
- 上传前断开 `STM32 PA2 -> ESP32-CAM U0R/RX`。
